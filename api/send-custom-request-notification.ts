...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    const {
      recipientEmail, recipientName, recipientType, actionType, cardCode, cardTitle,
      customerName, customerPhone, customerEmail, address, customRequestDetails,
      appointmentDate, appointmentTimeSlot, remarks, approvalNotes, portalUrl
    } = req.body || {};
  if (!recipientEmail) {
...
<tr>
  ...
  return res.status(result.success ? 200 : 500).json({
    ...
    console.error("Error sending custom request notification:", error);
    return res.status(500).json({ success: false, emailSent: false, error: error?.message || "Failed to send notification" });
  }
}
